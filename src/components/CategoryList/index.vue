...
export default {
  ...
  methods: {
    async getCategoryList() {
      ...
      if (code === 200) this.category1List = data
    },
    //更改一级菜单触发的事件
    async category1Change(id) {
      ...
      //将当前的category传递出去
      this.$emit('Category', {category: this.category1Id, level: 1})
    },
    //更改二级菜单触发的事件
    async category2Change(id) {
      ...
      //将当前的category传递出去
      this.$emit('Category', {category: this.category2Id, level: 2})
    },
    //更改三级菜单触发的事件
    category3Change(id) {
      //将当前的category传递出去
      this.$emit('Category', {category: this.category3Id, level: 3})
    }
  }
}
</script>
